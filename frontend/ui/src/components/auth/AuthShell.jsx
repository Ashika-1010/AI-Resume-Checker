  ...
  return (
    <div className="hidden lg:block flex-1 relative rounded-[28px] overflow-hidden isolate">
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(140deg, #0F1419 0%, #2C1610 38%, #1A0E0A 72%, #0a0705 100%)",
        }}
      />
      ...
      <motion.div
        ...
        style={{
          background:
            "radial-gradient(circle, rgba(196,91,58,0.55) 0%, transparent 70%)",
          filter: "blur(60px)",
        }}
        animate={{
          x: [0, 40, 0],
      ...
      <motion.div
        ...
        style={{
          background:
            "radial-gradient(circle, rgba(99,82,196,0.6) 0%, transparent 70%)",
          filter: "blur(60px)",
        }}
        animate={{
          x: [0, -30, 0],
...
export function AuthPrimaryButton({ children, disabled, ...props }) {
  return (
    <motion.button
      ...
      disabled={disabled}
      className="relative w-full h-12 rounded-2xl text-white font-semibold text-[15px] flex items-center justify-center gap-2 overflow-hidden shadow-[0_8px_24px_-8px_rgba(196,91,58,0.55)] transition-all duration-200 hover:shadow-[0_12px_28px_-8px_rgba(196,91,58,0.7)] disabled:opacity-60 disabled:cursor-not-allowed"
      style={{
        background:
          "linear-gradient(135deg, #C45B3A 0%, #A84830 50%, #8a3a26 100%)",
      }}
      {...props}
    >
      {/* Soft sheen on top */}
...
export function AuthErrorBanner({ children }) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      className="text-xs text-[var(--danger)] bg-[#FDEEE8] rounded-2xl px-4 py-2.5 leading-snug"
    >
      {children}
    </motion.div>
  );
